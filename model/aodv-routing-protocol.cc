...
public:
	...
	static TypeId GetTypeId() {
		static TypeId tid =
				TypeId("ns3::aodv::DeferredRouteOutputTag").SetParent<Tag>().SetParent<
						Tag>().AddConstructor<DeferredRouteOutputTag>();
		return tid;
	}
...
RoutingProtocol::RoutingProtocol() :
		RreqRetries(2), RreqRateLimit(10), RerrRateLimit(10), ActiveRouteTimeout(
				Seconds(3)), NetDiameter(35), NodeTraversalTime(
				MilliSeconds(40)), NetTraversalTime(
				Time((2 * NetDiameter) * NodeTraversalTime)), PathDiscoveryTime(
				Time(2 * NetTraversalTime)), MyRouteTimeout(
				Time(2 * std::max(PathDiscoveryTime, ActiveRouteTimeout))), HelloInterval(
				Seconds(1)), AllowedHelloLoss(2), DeletePeriod(
				Time(5 * std::max(ActiveRouteTimeout, HelloInterval))), NextHopWait(
				NodeTraversalTime + MilliSeconds(10)), BlackListTimeout(
				Time(RreqRetries * NetTraversalTime)), MaxQueueLen(64), MaxQueueTime(
				Seconds(30)), DestinationOnly(false), GratuitousReply(true), EnableHello(
				false), m_routingTable(DeletePeriod), m_queue(MaxQueueLen,
				MaxQueueTime), m_requestId(0), m_seqNo(0), m_rreqIdCache(
				PathDiscoveryTime), m_dpd(PathDiscoveryTime), m_nb(
				HelloInterval), m_rreqCount(0), m_rerrCount(0), m_htimer(
				Timer::CANCEL_ON_DESTROY), m_rreqRateLimitTimer(
				Timer::CANCEL_ON_DESTROY), m_rerrRateLimitTimer(
				Timer::CANCEL_ON_DESTROY), m_lastBcastTime(Seconds(0)) {
	m_nb.SetCallback(
...
TypeId RoutingProtocol::GetTypeId(void) {
	static TypeId tid =
			TypeId("ns3::aodv::RoutingProtocol").SetParent<Ipv4RoutingProtocol>().AddConstructor<
					...
					MakeTimeChecker()).AddAttribute("NextHopWait",
					...
					MakeTimeChecker()).AddAttribute("ActiveRouteTimeout",
					"Period of time during which the route is considered to be valid",
					TimeValue(Seconds(3)),
					MakeTimeAccessor(&RoutingProtocol::ActiveRouteTimeout),
					MakeTimeChecker()).AddAttribute("MyRouteTimeout",
					...
					MakeTimeChecker()).AddAttribute("BlackListTimeout",
					...
					MakeTimeChecker()).AddAttribute("PathDiscoveryTime",
					...
					MakeTimeChecker()).AddAttribute("MaxQueueLen",
					...
					MakeTimeChecker()).AddAttribute("AllowedHelloLoss",
					...
					BooleanValue(true),
					MakeBooleanAccessor(
							&RoutingProtocol::SetGratuitousReplyFlag,
					...
					MakeBooleanChecker()).AddAttribute("EnableHello",
					"Indicates whether a hello messages enable.",
					BooleanValue(true),
					MakeBooleanAccessor(&RoutingProtocol::SetHelloEnable,
							&RoutingProtocol::GetHelloEnable),
					MakeBooleanChecker()).AddAttribute("EnableBroadcast",
					"Indicates whether a broadcast data packets forwarding enable.",
					BooleanValue(true),
					MakeBooleanAccessor(&RoutingProtocol::SetBroadcastEnable,
							&RoutingProtocol::GetBroadcastEnable),
					MakeBooleanChecker()).AddAttribute("UniformRv",
					"Access to the underlying UniformRandomVariable",
					StringValue("ns3::UniformRandomVariable"),
					MakePointerAccessor(
							&RoutingProtocol::m_uniformRandomVariable),
...
Ptr<Ipv4Route> RoutingProtocol::RouteOutput(Ptr<Packet> p,
	...
	if (m_routingTable.LookupValidRoute(dst, rt)) {
		...
		for (std::vector<TrustTableEntry>::iterator it =
...
//direct trust calculation
		...
		std::vector<TrustTableEntry>& node_entry_vector =
				m_trustTable.getTrustTableEntries();

		for (std::vector<TrustTableEntry>::iterator it =
				node_entry_vector.begin(); it != node_entry_vector.end();
				it++) {
			double ind_trust_value = indTrustCal.calculateIndirectTrust(*it);
			it->updateIndirectTrust(ind_trust_value);
			it->calculateGlobalTrust();
...
void RoutingProtocol::DeferredRouteOutput(Ptr<const Packet> p,
	...
	if (result) {
		...
		if (!result || ((rt.GetFlag() != IN_SEARCH) && result)) {
			NS_LOG_LOGIC(
					"Send new RREQ for outbound packet to " <<header.GetDestination ());
		...
		if (m_ipv4->GetInterfaceForAddress(iface.GetLocal()) == iif)
			if (dst == iface.GetBroadcast() || dst.IsBroadcast()) {
				if (m_dpd.IsDuplicate(p, header)) {
					NS_LOG_DEBUG(
							"Duplicated packet " << p->GetUid () << " from " << origin << ". Drop.");
					return true;
				}
	...
	if (m_routingTable.LookupRoute(dst, toDst)) {
		if (toDst.GetFlag() == VALID) {
			...
			if (toDst.GetValidSeqNo()) {
				...
				NS_LOG_DEBUG(
						"Drop packet " << p->GetUid () << " because no route to forward it.");
				return false;
			}
		}
	}
...
void RoutingProtocol::SetIpv4(Ptr<Ipv4> ipv4) {
	...
	NS_ASSERT(
			m_ipv4->GetNInterfaces() == 1
					&& m_ipv4->GetAddress(0, 0).GetLocal()
							== Ipv4Address("127.0.0.1"));
	m_lo = m_ipv4->GetNetDevice(0);
	NS_ASSERT(m_lo != 0);
	// Remember lo route
	RoutingTableEntry rt(/*device=*/m_lo, /*dst=*/Ipv4Address::GetLoopback(), /*know seqno=*/
	true, /*seqno=*/0,
			/*iface=*/Ipv4InterfaceAddress(Ipv4Address::GetLoopback(),
					Ipv4Mask("255.0.0.0")),
...
void RoutingProtocol::NotifyInterfaceUp(uint32_t i) {
	...
	if (l3->GetNAddresses(i) > 1) {
		NS_LOG_WARN(
				"AODV does not work with more than one address per each interface.");
	}
	...
	if (l3->GetNAddresses(i) == 1) {
		...
		if (!socket) {
			...
			socket->SetRecvCallback(
					MakeCallback(&RoutingProtocol::RecvAodv, this));
			socket->BindToNetDevice(l3->GetNetDevice(i));
...
void RoutingProtocol::SendRequest(Ipv4Address dst) {
	...
	if (m_rreqCount == RreqRateLimit) {
		Simulator::Schedule(
				m_rreqRateLimitTimer.GetDelayLeft() + MicroSeconds(100),
	...
	if (m_routingTable.LookupRoute(dst, rt)) {
		...
		rreqHeader.SetUnknownSeqno(true);
		Ptr<NetDevice> dev = 0;
		RoutingTableEntry newEntry(/*device=*/dev, /*dst=*/dst, /*validSeqNo=*/
		false, /*seqno=*/0,
	...
	Ptr<OutputStreamWrapper> routingStream = Create<OutputStreamWrapper>(
			"aodv-routing_123.txt", std::ios::out);
	...
	for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
		...
		NS_LOG_DEBUG(
				"Send RREQ with id " << rreqHeader.GetId () << " to socket");
...
void RoutingProtocol::ScheduleRreqRetry(Ipv4Address dst) {
	...
	m_addressReqTimer[dst].SetFunction(
			&RoutingProtocol::RouteRequestTimerExpire, this);
	...
	NS_LOG_LOGIC(
			"Scheduled RREQ retry in " << Time (rt.GetRreqCnt () * NetTraversalTime).GetSeconds () << " seconds");
}
...
void RoutingProtocol::RecvAodv(Ptr<Socket> socket) {
	...
	NS_LOG_DEBUG(
			"AODV node " << this << " received a AODV packet from " << sender << " to " << receiver);
	...
	if (!tHeader.IsValid()) {
		NS_LOG_DEBUG(
				"AODV message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
		return; // drop
	}
	switch (tHeader.Get()) {
	case AODVTYPE_RREQ: {
...
void RoutingProtocol::UpdateRouteToNeighbor(Ipv4Address sender,
	...
	for (std::vector<RecommendationTableEntry>::iterator it =
			m_recommendationTable.getRecommendationTableEntries().begin();
			it != m_recommendationTable.getRecommendationTableEntries().end();
			it++) {
	...
	if (recCount == 0) {
		m_recommendationTable.addRecommendationTableEntry(
...
void RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver,
	...
	if (!m_routingTable.LookupRoute(origin, toOrigin)) {
		...
		true, /*seqNo=*/rreqHeader.GetOriginSeqno(),
				/*iface=*/m_ipv4->GetAddress(
						m_ipv4->GetInterfaceForAddress(receiver), 0), /*hops=*/
				hop,
		...
		if (toOrigin.GetValidSeqNo()) {
			if (int32_t(rreqHeader.GetOriginSeqno())
					- int32_t(toOrigin.GetSeqNo()) > 0)
	...
	NS_LOG_LOGIC(
			receiver << " receive RREQ with hop count " << static_cast<uint32_t>(rreqHeader.GetHopCount ()) << " ID " << rreqHeader.GetId () << " to destination " << rreqHeader.GetDst ());
	...
	if (m_routingTable.LookupRoute(dst, toDst)) {
		...
		if (toDst.GetNextHop() == src) {
			NS_LOG_DEBUG(
					"Drop RREQ from " << src << ", dest next hop " << toDst.GetNextHop ());
			return;
		}
		/*
...
void RoutingProtocol::SendReply(RreqHeader const & rreqHeader,
	...
	if (!rreqHeader.GetUnknownSeqno()
		...
		m_seqNo++;
	RrepHeader rrepHeader( /*prefixSize=*/0, /*hops=*/0, /*dst=*/
	rreqHeader.GetDst(),
...
void RoutingProtocol::SendReplyByIntermediateNode(RoutingTableEntry & toDst,
	...
	NS_LOG_FUNCTION(this);
	RrepHeader rrepHeader(/*prefix size=*/0, /*hops=*/toDst.GetHop(), /*dst=*/
	toDst.GetDestination(), /*dst seqno=*/toDst.GetSeqNo(),
	/*origin=*/toOrigin.GetDestination(), /*lifetime=*/
	toDst.GetLifeTime());
	...
	if (toDst.GetHop() == 1) {
		...
		toNextHop.m_ackTimer.SetFunction(&RoutingProtocol::AckTimerExpire,
				this);
	...
	if (gratRep) {
		...
		/*dst seqno=*/toOrigin.GetSeqNo(), /*origin=*/
...
void RoutingProtocol::RecvReply(Ptr<Packet> p, Ipv4Address receiver,
	...
	/*iface=*/m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(receiver), 0),/*hop=*/
	hop,
	...
	NS_LOG_LOGIC(
			"receiver " << receiver << " origin " << rrepHeader.GetOrigin ());
	...
	TrustTableEntry* trustTableEntry = m_trustTable.getTrustTableEntryByNodeId(
			toDst.GetDestination());
...
void RoutingProtocol::ProcessHello(RrepHeader const & rrepHeader,
	...
	if (!m_routingTable.LookupRoute(rrepHeader.GetDst(), toNeighbor)) {
		...
		true, /*seqno=*/rrepHeader.GetDstSeqno(),
				/*iface=*/m_ipv4->GetAddress(
						m_ipv4->GetInterfaceForAddress(receiver), 0),
		...
		toNeighbor.SetLifeTime(
				std::max(Time(AllowedHelloLoss * HelloInterval),
...
void RoutingProtocol::RecvError(Ptr<Packet> p, Ipv4Address src) {
	...
	for (std::map<Ipv4Address, uint32_t>::const_iterator i =
			unreachable.begin(); i != unreachable.end();) {
...
void RoutingProtocol::RouteRequestTimerExpire(Ipv4Address dst) {
	...
	if (toDst.GetRreqCnt() == RreqRetries) {
		NS_LOG_LOGIC(
				"route discovery to " << dst << " has been attempted RreqRetries (" << RreqRetries << ") times");
		m_addressReqTimer.erase(dst);
...
void RoutingProtocol::SendHello() {
	...
	for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
		...
		TrustTableEntry* trustTableEntry =
				m_trustTable.getTrustTableEntryByNodeId(destination);
		if (trustTableEntry != 0) {
...
void RoutingProtocol::SendRerrWhenNoRouteToForward(Ipv4Address dst,
	...
	if (m_rerrCount == RerrRateLimit) {
		...
		NS_LOG_LOGIC(
				"RerrRateLimit reached at " << Simulator::Now ().GetSeconds () << " with timer delay left " << m_rerrRateLimitTimer.GetDelayLeft ().GetSeconds () << "; suppressing RERR");
		return;
	}
	RerrHeader rerrHeader;
...
void RoutingProtocol::SendRerrMessage(Ptr<Packet> packet,
	...
	if (precursors.size() == 1) {
		...
		if (m_routingTable.LookupValidRoute(precursors.front(), toPrecursor)) {
			...
			NS_ASSERT(socket);
			NS_LOG_LOGIC(
					"one precursor => unicast RERR to " << toPrecursor.GetDestination () << " from " << toPrecursor.GetInterface ().GetLocal ());
			Simulator::Schedule(
					Time(
							MilliSeconds(
									m_uniformRandomVariable->GetInteger(0,
											10))), &RoutingProtocol::SendTo,
					this, socket, packet, precursors.front());
			m_rerrCount++;

			TrustTableEntry* trustTableEntry =
					m_trustTable.getTrustTableEntryByNodeId(
							toPrecursor.GetDestination());
	...
	for (std::vector<Ipv4Address>::const_iterator i = precursors.begin();
		...
		if (m_routingTable.LookupValidRoute(*i, toPrecursor)
				&& std::find(ifaces.begin(), ifaces.end(),
						toPrecursor.GetInterface()) == ifaces.end()) {
 ...
 * targetNode: D
 */
void RoutingProtocol::sendTRR(Ipv4Address source, Ipv4Address receiver,
...
//	std::cout<<"Dest "<<trrHeader.GetDst()<<" Source "<<trrHeader.GetOrigin()<<" targetNode "<<trrHeader.GetTarget()<<std::endl;
...
void RoutingProtocol::RecvTrr(Ipv4Address sender, Ptr<Packet> packet) {
	RoutingTableEntry rt;
	TRRHeader trrHeader;
	...
	if (status) {
		...
		if (m_routingTable.LookupValidRoute(trrHeader.GetOrigin(),
			...
			Ptr<Socket> socket = FindSocketWithInterfaceAddress(
					searchingRoutingEntry.GetInterface());
			Simulator::Schedule(
					Time(
							MilliSeconds(
									m_uniformRandomVariable->GetInteger(0,
											10))), &RoutingProtocol::SendTo,
					this, socket, packetReply, sender);

			if (m_TRRTable.getTrrTableEntries().size() == nodeCount) {
				std::cout
						<< "\n  ================== Printing TRR table =================="
						<< std::endl;
				m_TRRTable.printTable();
...
void RoutingProtocol::execute() {
	...
	for (std::vector<TrustTableEntry>::iterator mainItr =
			m_trustTable.getTrustTableEntries().begin();
			mainItr != m_trustTable.getTrustTableEntries().end(); mainItr++) {
		Ipv4Address selectedTarget = mainItr->getDestinationNode();

		for (std::vector<TrustTableEntry>::iterator it2 =
				m_trustTable.getTrustTableEntries().begin();
				it2 != m_trustTable.getTrustTableEntries().end(); it2++) {

			if (it2->getDestinationNode() != selectedTarget) {
				sendTRR(iface.GetLocal(), it2->getDestinationNode(),
						selectedTarget);
			}
		}
	}
	std::cout
			<< "\n  ================== Printing trust table =================="
			<< std::endl;
//	m_recommendationTable.printTable();
	m_trustTable.printTable();
}

void RoutingProtocol::setNodeId() {
	Ipv4InterfaceAddress iface1;
	for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
			m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j) {
		Ptr<Socket> socket = j->first;
		iface1 = j->second;
		nodeIpv4.push_back(iface1.GetLocal());
	}
}
