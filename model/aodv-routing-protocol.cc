...
void RoutingProtocol::DeferredRouteOutput(Ptr<const Packet> p,
	...
	if (result) {
		...
		if (!result || ((rt.GetFlag() != IN_SEARCH) && result)) {
			NS_LOG_LOGIC(
					"Send new RREQ for outbound packet to "
		...
		if (m_ipv4->GetInterfaceForAddress(iface.GetLocal()) == iif)
			if (dst == iface.GetBroadcast() || dst.IsBroadcast()) {
				if (m_dpd.IsDuplicate(p, header)) {
					NS_LOG_DEBUG(
							"Duplicated packet " << p->GetUid() << " from "
									<< origin << ". Drop.");
					return true;
				}
				...
				if (header.GetTtl() > 1) {
					NS_LOG_LOGIC(
							"Forward broadcast. TTL "
									<< (uint16_t) header.GetTtl());
					...
					if (m_routingTable.LookupRoute(dst, toBroadcast)) {
						...
						NS_LOG_DEBUG(
								"No route to forward broadcast. Drop packet "
										<< p->GetUid());
					}
				} else {
...
bool RoutingProtocol::Forwarding(Ptr<const Packet> p, const Ipv4Header & header,
	...
	if (m_routingTable.LookupRoute(dst, toDst)) {
		if (toDst.GetFlag() == VALID) {
			...
			NS_LOG_LOGIC(
					route->GetSource() << " forwarding to " << dst << " from "
...
void RoutingProtocol::RecvAodv(Ptr<Socket> socket) {
	...
	NS_LOG_DEBUG(
			"AODV node " << this << " received a AODV packet from " << sender
					<< " to " << receiver);
...
void RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver,
	...
	NS_LOG_LOGIC(
			receiver << " receive RREQ with hop count "
					...
					<< rreqHeader.GetId() << " to destination "
					<< rreqHeader.GetDst());
	...
	if (m_routingTable.LookupRoute(dst, toDst)) {
		...
		if (toDst.GetNextHop() == src) {
			NS_LOG_DEBUG(
					"Drop RREQ from " << src << ", dest next hop "
							<< toDst.GetNextHop());
			return;
		}
		/*
...
void RoutingProtocol::RecvReply(Ptr<Packet> p, Ipv4Address receiver,
	...
	NS_LOG_LOGIC(
			"RREP destination " << dst << " RREP origin "
...
void RoutingProtocol::RouteRequestTimerExpire(Ipv4Address dst) {
	...
	if (toDst.GetRreqCnt() == RreqRetries) {
		NS_LOG_LOGIC(
				"route discovery to " << dst
						<< " has been attempted RreqRetries (" << RreqRetries
						<< ") times");
		m_addressReqTimer.erase(dst);
	...
	if (toDst.GetFlag() == IN_SEARCH) {
		...
		NS_LOG_DEBUG(
				"Route down. Stop search. Drop packet with destination "
						<< dst);
		m_addressReqTimer.erase(dst);
		m_routingTable.DeleteRoute(dst);
		m_queue.DropPacketWithDst(dst);
...
void RoutingProtocol::SendRerrWhenNoRouteToForward(Ipv4Address dst,
	...
	if (m_rerrCount == RerrRateLimit) {
		...
		NS_LOG_LOGIC(
				"RerrRateLimit reached at " << Simulator::Now().GetSeconds()
	...
	if (m_routingTable.LookupValidRoute(origin, toOrigin)) {
		...
		for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
			...
			NS_ASSERT(socket);
			NS_LOG_LOGIC(
					"Broadcast RERR message from interface "
							<< iface.GetLocal());
			// Send to all-hosts broadcast if on /32 addr, subnet-directed otherwise
			Ipv4Address destination;
			if (iface.GetMask() == Ipv4Mask::GetOnes()) {
	...
	if (status) {
		...
		if (m_routingTable.LookupValidRoute(trrHeader.GetOrigin(),
			...
			if (m_TRRTable.getTrrTableEntries().size() == nodeCount) {
				...
				std::cout
						<< "\n  ================== Printing Trust table after TRR process=================="
						<< std::endl;
...
void RoutingProtocol::execute() {
	...
	std::cout
			<< "\n  ================== Printing trust table before TRR process=================="
			<< std::endl;
