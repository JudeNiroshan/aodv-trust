...
#include "ns3/aodv-trust-module.h"
...
AodvExample::AodvExample() :
		size(4), step(20), totalTime(10), pcap(true), printRoutes(true) {
}
...
void AodvExample::PrintTble() {
	std::string stringLine;
	std::ifstream infile;
	infile.open("aodv-routing.txt");
	std::cout << std::endl;
	while (!infile.eof()) // To get all the lines.
	{
		getline(infile, stringLine); // Saves the line in stringLine.
		std::cout << stringLine << std::endl; // Prints our stringLine.
	}
	infile.close();
}
...
void AodvExample::CreateNodes() {
	...
	mobility.SetPositionAllocator("ns3::GridPositionAllocator", "MinX",
			...
			DoubleValue(step), "DeltaY", DoubleValue(0), "GridWidth",
			UintegerValue(size), "LayoutType", StringValue("RowFirst"));
