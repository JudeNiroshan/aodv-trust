...
TypeId TypeHeader::GetTypeId() {
	static TypeId tid =
			TypeId("ns3::aodv::TypeHeader").SetParent<Header>().AddConstructor<
					TypeHeader>();
	return tid;
}
...
TypeId RreqHeader::GetTypeId() {
	static TypeId tid =
			TypeId("ns3::aodv::RreqHeader").SetParent<Header>().AddConstructor<
					RreqHeader>();
	return tid;
}
...
TypeId RrepHeader::GetTypeId() {
	static TypeId tid =
			TypeId("ns3::aodv::RrepHeader").SetParent<Header>().AddConstructor<
					RrepHeader>();
	return tid;
}
...
TypeId RrepAckHeader::GetTypeId() {
	static TypeId tid =
			TypeId("ns3::aodv::RrepAckHeader").SetParent<Header>().AddConstructor<
					RrepAckHeader>();
	return tid;
}
...
TypeId RerrHeader::GetTypeId() {
	static TypeId tid =
			TypeId("ns3::aodv::RerrHeader").SetParent<Header>().AddConstructor<
					RerrHeader>();
	return tid;
}
...
TRRHeader::TRRHeader(uint32_t GT, uint32_t DT, uint32_t trrID, Ipv4Address dst,
		uint32_t dstSeqNo, Ipv4Address origin, Ipv4Address target,
		uint32_t originSeqNo) :
		m_GT(GT), m_DT(DT), m_trrID(trrID), m_dst(dst), m_dstSeqNo(dstSeqNo), m_origin(
				origin), m_target(target), m_originSeqNo(originSeqNo) {
}

NS_OBJECT_ENSURE_REGISTERED(TRRHeader);

TypeId TRRHeader::GetTypeId() {
	static TypeId tid =
			TypeId("ns3::aodv::TRRHeader").SetParent<Header>().AddConstructor<
					TRRHeader>();
	return tid;
}

TypeId TRRHeader::GetInstanceTypeId() const {
	return GetTypeId();
}

uint32_t TRRHeader::GetSerializedSize() const {
	return 28;
}

void TRRHeader::Serialize(Buffer::Iterator i) const {
	i.WriteU16(m_GT);
	WriteTo(i, m_dst);
	i.WriteU8(m_DT);
...
uint32_t TRRHeader::Deserialize(Buffer::Iterator start) {
	Buffer::Iterator i = start;
	m_GT = i.ReadU16();
	ReadFrom(i, m_dst);
	m_DT = i.ReadU8();
...
void TRRHeader::Print(std::ostream &os) const {
	os << "TRR ID " << m_trrID << " destination: ipv4 " << m_dst
			<< " sequence number " << m_dstSeqNo << " source: ipv4 " << m_origin
			<< " target: ipv4 " << m_target << " sequence number "
			<< m_originSeqNo;
}
...
bool TRRHeader::operator==(TRRHeader const & o) const {
	return (m_GT == o.m_GT && m_DT == o.m_DT && m_dst == o.m_dst
			&& m_dstSeqNo == o.m_dstSeqNo && m_origin == o.m_origin
			&& m_target == o.m_target && m_originSeqNo == o.m_originSeqNo);
}
}
}
