...
TrustTableEntry* TrustTable::getTrustTableEntryByNodeId(Ipv4Address nodeId) {
	...
	for (std::vector<TrustTableEntry>::iterator it = trustTableRecords.begin();
			it != trustTableRecords.end(); it++) {
...
TrustTableEntry TrustTable::getTrustTableEntryByNodeIdCopy(Ipv4Address nodeId) {
...
void TrustTable::printTable() {
	std::cout << ">>>>>>>>>>>>>> Direct Trust Table <<<<<<<<<<<<<<<"
			<< std::endl;
	std::cout << "| " << "Destination Node" << columnSeperator << "Direct Trust"
			<< columnSeperator << "Indirect Trust" << columnSeperator
			<< "Interactions" << columnSeperator << "Global Trust"
			<< columnSeperator << "Trust Level" << columnSeperator << "RREQ"
			<< columnSeperator << "RPLY" << columnSeperator << "Hello"
			<< columnSeperator << "ERR" << columnSeperator << "NDR"
			<< columnSeperator << "NDF" << std::endl;
	for (std::vector<TrustTableEntry>::iterator it = trustTableRecords.begin();
			it != trustTableRecords.end(); it++) {
		std::cout << "| " << it->getDestinationNode() << "\t" << columnSeperator
				<< it->getDirectTrust() << "\t" << columnSeperator
				<< it->getIndirectTrust() << "\t" << columnSeperator
				<< it->getInteractionCount() << "\t" << columnSeperator
				<< it->getGlobalTrust() << "\t" << columnSeperator
				<< it->getTrustLevel() << "\t" << columnSeperator
				<< it->getNoOfRREQ() << "\t" << columnSeperator
				<< it->getNoOfRPLY() << "\t" << columnSeperator
				<< it->getNoOfHELLO() << "\t" << columnSeperator
				<< it->getNoOfERR() << "\t" << columnSeperator
				<< it->getNoOfReceivedDataPackets() << "\t" << columnSeperator
				<< it->getNoOfSentDataPackets() << std::endl;
	}
}
